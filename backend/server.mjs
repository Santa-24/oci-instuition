...
// 2. Seed Baseline Academic Data if empty
const courseCount = db.prepare('SELECT COUNT(*) as count FROM courses').get().count;
if (courseCount === 0) {
  console.log('[Database] Seeding initial academic master data...');
  
  // Courses
  const insertCourse = db.prepare(`
    INSERT INTO courses (id, name, code, category, duration_months, description)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
  insertCourse.run('crs_01', 'JEE Advanced 2 Year Comprehensive', 'JEE-ADV-2Y', 'Engineering', 24, 'Complete 2-year classroom program for Class 11 & 12.');
  insertCourse.run('crs_02', 'NEET Medical Pioneer 2 Year', 'NEET-MED-2Y', 'Medical', 24, 'Flagship 2-year medical preparation for NEET-UG.');
  insertCourse.run('crs_03', 'Foundation Olympiad & NTSE', 'FND-OLY-1Y', 'Foundation', 12, 'Strengthening STEM fundamentals for Class 9 & 10.');

  // Subjects
  const insertSubject = db.prepare('INSERT INTO subjects (id, course_id, name, code) VALUES (?, ?, ?, ?)');
  insertSubject.run('sbj_01', 'crs_01', 'Physics (JEE Advanced)', 'PHY-01');
  insertSubject.run('sbj_02', 'crs_01', 'Chemistry (JEE Advanced)', 'CHE-01');
  insertSubject.run('sbj_03', 'crs_01', 'Mathematics (JEE Advanced)', 'MAT-01');
  insertSubject.run('sbj_04', 'crs_02', 'Botany & Zoology (NEET)', 'BIO-01');

  // Batches
  const insertBatch = db.prepare(`
    INSERT INTO batches (id, course_id, name, schedule, room_name, start_date, end_date, capacity)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
  insertBatch.run('batch_alpha', 'crs_01', 'JEE Alpha Super 30', 'Mon-Fri 08:00 AM - 01:30 PM', 'Hall A (Aryabhata)', '2026-04-01', '2028-03-31', 30);
  insertBatch.run('batch_delta', 'crs_01', 'JEE Delta Achievers', 'Mon-Fri 02:00 PM - 07:30 PM', 'Hall B (Ramanujan)', '2026-04-01', '2028-03-31', 45);
  insertBatch.run('batch_neet', 'crs_02', 'NEET Medical Premier', 'Mon-Sat 08:00 AM - 02:00 PM', 'Hall C (Sushruta)', '2026-04-01', '2028-03-31', 40);

  // Users & Profiles: Admin, Faculty, Student
  const insertProfile = db.prepare('INSERT INTO profiles (id, email, full_name, phone) VALUES (?, ?, ?, ?)');
  const insertRole = db.prepare('INSERT INTO user_roles (id, user_id, role) VALUES (?, ?, ?)');
  
  // Admin
  insertProfile.run('usr_admin_01', 'admin@oci.edu.in', 'Director Dr. S. K. Nayak', '+91 94370 12345');
  insertRole.run('role_01', 'usr_admin_01', 'admin');

  // Faculty
  insertProfile.run('usr_fac_01', 'faculty.verma@oci.edu.in', 'Prof. Arvind Verma (HOD Physics)', '+91 98610 23456');
  insertRole.run('role_02', 'usr_fac_01', 'teacher');
  const insertTeacher = db.prepare(`
    INSERT INTO teachers (id, employee_id, subject, qualification, experience_years, bio)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
  insertTeacher.run('usr_fac_01', 'EMP-PHY-101', 'Physics', 'M.Tech IIT Kharagpur', 14, 'Specialist in Classical Mechanics & Electrodynamics.');

  // Student
  insertProfile.run('usr_std_01', 'student@oci.edu.in', 'Aarav Pattnaik', '+91 70081 34567');
  insertRole.run('role_03', 'usr_std_01', 'student');
  const insertStudent = db.prepare(`
    INSERT INTO students (id, roll_no, batch_id, admission_date, status)
    VALUES (?, ?, ?, ?, ?)
  `);
  insertStudent.run('usr_std_01', 'OCI-2026-0042', 'batch_alpha', '2026-04-05', 'active');

  // Live Classes
  const insertLive = db.prepare(`
    INSERT INTO live_classes (id, batch_id, teacher_id, subject, title, scheduled_start, scheduled_end, jitsi_room_name, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  insertLive.run('live_01', 'batch_alpha', 'usr_fac_01', 'Physics', 'Rotational Mechanics: Angular Momentum Conservation', '2026-09-18T10:00:00Z', '2026-09-18T11:30:00Z', 'OCI_ROOM_ALPHA_PHY_101', 'scheduled');

  // Exams
  const insertExam = db.prepare(`
    INSERT INTO exams (id, course_id, title, duration_minutes, total_marks, is_published, scheduled_date)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  insertExam.run('exam_01', 'crs_01', 'All India JEE Advanced Full Mock #4', 180, 300, 1, '2026-09-20T09:00:00Z');

  // Questions for exam
  const insertQ = db.prepare(`
    INSERT INTO questions (id, subject, topic, question, options, correct_option_index, explanation, difficulty, marks, negative_marks)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  insertQ.run(
    'q_01',
    'Physics',
    'Rotational Dynamics',
    'A solid sphere rolls without slipping down an inclined plane of angle theta. The minimum coefficient of static friction required is:',
    JSON.stringify(['(2/7) tan(theta)', '(1/3) tan(theta)', '(2/5) tan(theta)', '(1/2) tan(theta)']),
    0,
    'For a solid sphere, I = 2/5 mR^2. a = g sin(theta)/(1 + I/mR^2) = 5/7 g sin(theta). fs = mg sin(theta) - ma = 2/7 mg sin(theta) <= mu mg cos(theta). Therefore mu >= 2/7 tan(theta).',
    'Medium',
    4,
    1
  );
  insertQ.run(
    ...
    'Mathematics',
    'Calculus',
    'The value of the integral from 0 to 1 of (x^4 (1-x)^4)/(1 + x^2) dx is equal to:',
    JSON.stringify(['22/7 - pi', 'pi - 22/7', '355/113 - pi', 'pi/4']),
    0,
    'This is the celebrated Dalzell integral proving 22/7 > pi.',
    'Hard',
    4,
    1
  );
  ...
  const insertAssign = db.prepare(`
    INSERT INTO assignments (id, batch_id, subject, title, description, due_date, status)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  insertAssign.run('asg_01', 'batch_alpha', 'Physics', 'Problem Set 4: Gyroscopic Precession & Rigid Bodies', 'Solve problems 14 through 28 from Irodov Mechanics.', '2026-09-22T23:59:59Z', 'active');

  // Announcements
  const insertAnnounce = db.prepare(`
    INSERT INTO announcements (id, title, content, category, is_urgent)
    VALUES (?, ?, ?, ?, ?)
  `);
  insertAnnounce.run('ann_01', 'JEE Advanced 2027 Registration Schedule Update', 'Official portal registration begins next Monday. Check notice board for student ID verification.', 'Academic', 1);

  // Study Materials
  const insertMat = db.prepare(`
    INSERT INTO study_materials (id, batch_id, subject, title, type, file_url, file_size)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  insertMat.run('mat_01', 'batch_alpha', 'Physics', 'Advanced Electrostatics Formula Sheet & Key Insights', 'PDF', 'https://oci.edu.in/materials/electrostatics_formulas.pdf', '4.2 MB');
